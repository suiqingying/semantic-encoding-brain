\chapter{多模态模型结果：Whisper 与 CLAP 的对比}
多模态模型部分的结果来自 \texttt{results/summary.csv} 中 \texttt{/multimodal/} 相关条目以及对应目录下的 \texttt{corr\_layer*.npy}。图 \ref{fig:multi_best} 展示了各多模态模型在已完成条件下的最佳层均值相关，图 \ref{fig:multi_tr_trend} 展示了不同 TR 窗口下的最佳层趋势。当前结果中，\texttt{openai/whisper-base} 在 6TR、layer2 达到 0.0889，已经非常接近强音频基线 Wav2Vec2 的 0.0895；\texttt{openai/whisper-small} 在 6TR、layer9 达到 0.0844；\texttt{laion/clap-htsat-unfused} 在 6TR、layer1 达到 0.0788。值得注意的是，Whisper-base 在 1TR、2TR、3TR 条件下也有多层完成记录，其最佳均值分别为 0.0270、0.0408、0.0579，趋势与音频模型一致，说明多模态模型同样高度依赖较长时间窗的聚合。
...
\InsertFig{multimodal_class_montage.png}{0.96\linewidth}{多模态模型类别脑图对照：Whisper-base（6TR，layer2）、Whisper-small（6TR，layer9）、CLAP（6TR，layer1）（由 src/run\_plot\_corr\_maps.py 绘制并组合）。}{fig:multi_montage}
...
\InsertFig{multimodal_laion_clap-htsat-unfused_6TR_corr_layer1.png}{0.92\linewidth}{CLAP-HTSAT-unfused（6TR，layer1）相关图可视化（由 src/run\_plot\_corr\_maps.py 从 corr\_layer1.npy 绘制）。}{fig:multi_clap_map}
