...
\subsection{非线性编码模型补充：Kernel Ridge}
除线性岭回归外，我们还在文本对齐特征上补充评估了一个非线性基线：核岭回归（Kernel Ridge）。该模型以 \texttt{results/text/*/win*/aligned\_layer*.npy} 保存的 TR 级对齐特征为输入（无需再次运行预训练模型前向），并在相同的 FIR 时延展开（默认 $W=4$、$O=1$）后执行训练/测试划分与评估。具体而言，对每个被试，我们同样在两端排除 10 个 TR，并按时间顺序做 80\%/20\% 划分；在训练集上拟合 Kernel Ridge，在测试集上预测并计算每个 ROI 的相关系数，再对 ROI 维取平均得到该被试的平均相关。当前结果使用 RBF 核，超参数为 \texttt{alpha=1.0}、\texttt{gamma=None}（由实现使用默认策略确定）。与线性模型不同，非线性基线在当前实现中仅输出每组特征的多被试平均相关统计到 \texttt{results/nonlinear/log.txt}，并未额外保存 corr map，因此本文在非线性部分只报告可追溯的数值统计，不加入新的脑图对比。
