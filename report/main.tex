...
% 成员信息块（用于“成员与分工”表格的左侧信息列）
\newcommand{\MemberInfo}[3]{%
  \parbox[t]{\linewidth}{%
    \vspace{0pt}% 强制与右栏顶部对齐
    {\TitleCJK\bfseries\color{Nord10}#1}\par
    {\sffamily\small\color{Nord3}学号：#2}\par
    {\sffamily\small\color{Nord3}邮箱：\urlstyle{same}%
      \begingroup\def\UrlBreaks{\do\/\do\-\do\.\do\_}\nolinkurl{#3}\endgroup}%
  }%
}
...
% 项目开源仓库
\newcommand{\RepoURL}{https://github.com/suiqingying/semantic-encoding-brain}

\newcommand{\PaperTitle}{自然故事听觉范式下的语义编码}
\newcommand{\PaperSubtitle}{预训练文本、语音与多模态表征的比较与融合（基于 The 21st Year fMRI 数据）}
\newcommand{\PaperTitleEn}{Semantic encoding in natural story listening: comparing and fusing pretrained text, speech, and multimodal representations}
